...
async function testFetchSubscriptionFromDB(subscriptionId, userToken, planFrequency) {
  ...
  try {
    ...
    const response = await makeRequest('GET', `/api/subscriptions/${subscriptionId}`, userToken);
    ...
    let parsedAddress;
    try {
      parsedAddress = typeof sub.address === 'string' ? JSON.parse(sub.address) : sub.address;
...
async function testDeliveryLogs(subscriptionId, userToken) {
...
async function testScheduleEndpoint(subscriptionId, userToken) {
...
async function runAllTests() {
  ...
  try {
    ...
    await testFetchSubscriptionFromDB(test1.subscriptionId, test1.userToken, test1.planFrequency);
