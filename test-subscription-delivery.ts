...
function makeRequest(method: string, path: string, token?: string, body?: any): Promise<any> {
...
async function testAdminSubscriptionsDateFormat() {
  ...
  try {
    ...
    const paidSub = subscriptions.find((s: any) => s.isPaid === true);
    if (!paidSub) {
      results.push({ 
        name, 
        passed: true, 
...
function testDateSerializationLogic() {
  ...
  try {
    ...
    function testSerialize(dateInput: any): { valid: boolean; result: string | null } {
      if (!dateInput) {
