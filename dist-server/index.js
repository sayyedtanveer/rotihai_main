...
__export(schema_exports, {
  ...
  insertPendingCheckoutSchema: () => insertPendingCheckoutSchema,
...
var init_schema = __esm({
  "shared/schema.ts"() {
    ...
    payoutTransactions = pgTable("payout_transactions", {
      id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
    ...
    pendingCheckouts = pgTable("pending_checkouts", {
      ...
      items: jsonb("items").notNull(),
      // Array of {id, name, price, quantity, categoryId, chefId, ...}
...
[DB-DEBUG] getSubscription(${id}) - Raw DB response:`);
      ...
      async getChefReport(from, to, chefId) {
        try {
          ...
          const chefStats = targetChefs.map((chef) => {
            const orders3 = chefOrders.filter((o) => o.chefId === chef.id);
            let chefEarnings = 0;
            const productSales = /* @__PURE__ */ new Map();
            ...
            const ordersCount = orders3.length;
            const avgEarning = ordersCount > 0 ? Math.round(chefEarnings / ordersCount) : 0;
            const topProducts = Array.from(productSales.entries()).map(([id, data]) => ({ id, ...data })).sort((a, b) => b.revenue - a.revenue).slice(0, 3);
            return {
              id: chef.id,
              name: chef.name,
              chefEarnings,
              // Changed from totalRevenue
              totalOrders: ordersCount,
              averageEarning: avgEarning,
              // Changed from averageOrderValue
              topProducts,
          ...
          const totalChefEarnings = chefOrders.reduce((sum, o) => {
            const orderChefEarnings = o.items.reduce((itemSum, item) => {
      ...
      async getRothiaiEarningsReport(from, to) {
        try {
          const allOrders = await db.query.orders.findMany();
          const filteredOrders = allOrders.filter((o) => {
            const createdAt = new Date(o.createdAt);
          ...
          const categoryEarnings = /* @__PURE__ */ new Map();
          for (const order of filteredOrders) {
          ...
          const categoryBreakdown = Array.from(categoryEarnings.values()).sort((a, b) => b.earnings - a.earnings);
          return {
            totalOrders: filteredOrders.length,
            grossRothiaiEarnings,
            totalRothiaiEarnings,
            // NET after referral bonuses
            breakdown: {
              platformCommission,
              deliveryFeeEarnings,
              discountTaken,
              walletUsed,
              referralBonusesSpent
              // NEW: What platform paid out for referrals
            },
            categoryBreakdown
          };
        } catch (error) {
      ...
      async getChefPayoutDetails(from, to, chefId) {
        try {
          const allOrders = await db.query.orders.findMany();
          const filteredOrders = allOrders.filter((o) => {
            const createdAt = new Date(o.createdAt);
      ...
      async createChefPayout(chefId, orderId, amount, paymentMethod) {
        try {
          const payout = await db.insert(payoutTransactions2).values({
            chefId,
            orderId,
            amount,
            status: "pending",
      ...
      async markChefPayoutAsPaid(payoutId, transactionId) {
        try {
          const updated = await db.update(payoutTransactions2).set({
            status: "paid",
            paidAt: /* @__PURE__ */ new Date(),
            transactionId
          }).where(eq(payoutTransactions2.id, payoutId)).returning();
          return updated[0];
        } catch (error) {
      ...
      async markMultiplePayoutsAsPaid(payoutIds) {
        try {
          const updated = await db.update(payoutTransactions2).set({
            status: "paid",
            paidAt: /* @__PURE__ */ new Date()
          }).where(inArray(payoutTransactions2.id, payoutIds)).returning();
          return updated;
        } catch (error) {
      ...
      async rejectOrder(orderId, rejectedBy, reason) {
        ...
        if (updatedOrder && updatedOrder.userId && updatedOrder.referralCode) {
          try {
            ...
            if (referral) {
              console.log(`\u{1F381} [REFERRAL CLAWBACK] Found pending referral: ${referral.id} - Status: ${referral.status}`);
      ...
      async expireOldPendingReferrals() {
        try {
          ...
          const expiryDate = new Date(Date.now() - expiryDays * 24 * 60 * 60 * 1e3);
          console.log(`\u{1F550} [REFERRAL EXPIRY] Expiring referrals older than ${expiryDays} days (before ${expiryDate.toISOString()})`);
          const result = await db.update(referrals2).set({
            status: "expired"
          }).where(and(
            eq(referrals2.status, "pending"),
            lt(referrals2.createdAt, expiryDate)
          )).returning();
...
\u{1F512} [ATOMIC-PHASE2] Starting atomic payment + wallet transaction for order ${orderId}...`);
      ...
      async getAllCoupons() {
        return db.query.coupons.findMany({
          orderBy: (c, { desc: desc3 }) => [desc3(c.createdAt)]
        });
      }
      // Create coupon
      ...
      async getCouponStats() {
        ...
        for (const coupon of allCoupons) {
          ...
          stats.push({
            ...
            uniqueUsers,
            totalDiscount: 0,
            // Would need order data to calculate accurately
            avgDiscount: 0,
            // Would need order data to calculate accurately
            lastUsed
          });
          ...
          if (referral.referrerBonus > 0) {
            try {
              const referrerUser = await tx.query.users.findFirst({
                where: eq(users2.id, referral.referrerId)
              });
              if (!referrerUser) {
          ...
          if (referral.referredBonus > 0) {
            try {
              const referredUser = await tx.query.users.findFirst({
                where: eq(users2.id, referral.referredId)
              });
              if (!referredUser) {
      ...
      async savePendingCheckout(data) {
        ...
        await db.insert(pendingCheckouts2).values(pending);
      ...
      async updatePendingCheckout(id, data) {
        await db.update(pendingCheckouts2).set({ ...data, updatedAt: /* @__PURE__ */ new Date() }).where(eq(pendingCheckouts2.id, id));
        return this.getPendingCheckout(id);
      }
      async markPendingCheckoutAsConfirmed(id, orderId) {
        return this.updatePendingCheckout(id, {
          status: "confirmed",
          orderId
        });
      }
      async markPendingCheckoutAsConfirmedAndDeleted(id, orderId) {
        ...
        if (otherCheckouts.length > 0) {
          const otherIds = otherCheckouts.filter((pc) => pc.id !== id).map((pc) => pc.id);
          if (otherIds.length > 0) {
            ...
            await db.update(pendingCheckouts2).set({
              status: "abandoned",
              updatedAt: /* @__PURE__ */ new Date()
            }).where(inArray(pendingCheckouts2.id, otherIds));
        ...
        await db.update(pendingCheckouts2).set({
          ...
          isDeleted: true,
          updatedAt: /* @__PURE__ */ new Date()
        }).where(eq(pendingCheckouts2.id, id));
      ...
      async deletePendingCheckout(id) {
        const result = await db.delete(pendingCheckouts2).where(eq(pendingCheckouts2.id, id));
...
__export(userAuth_exports, {
  ...
  generateRefreshToken: () => generateRefreshToken2,
...
function generateAccessToken2(user) {
  const payload = {
    userId: user.id,
    phone: user.phone,
    name: user.name
...
function generateRefreshToken2(user) {
  const payload = {
    userId: user.id,
    phone: user.phone,
    name: user.name
...
var init_userAuth = __esm({
  "server/userAuth.ts"() {
    "use strict";
    JWT_SECRET2 = process.env.JWT_SECRET || "user-jwt-secret-change-in-production";
    JWT_EXPIRES_IN2 = "7d";
...
async function hashPassword3(password) {
  ...
  return bcrypt3.hash(password, salt);
}
...
function generateDeliveryToken(deliveryPerson) {
  ...
  return jwt3.sign(payload, JWT_SECRET3, { expiresIn: JWT_EXPIRES_IN3 });
}
function generateRefreshToken3(deliveryPerson) {
  const payload = {
    deliveryId: deliveryPerson.id,
    name: deliveryPerson.name,
    phone: deliveryPerson.phone
  };
  return jwt3.sign(payload, JWT_SECRET3, { expiresIn: REFRESH_TOKEN_EXPIRES_IN3 });
}
function verifyToken3(token) {
  try {
    const payload = jwt3.verify(token, JWT_SECRET3);
    console.log("[DELIVERY-VERIFY-TOKEN] \u2705 Token verified with JWT_SECRET:", JWT_SECRET3.substring(0, 10) + "...");
    return payload;
  } catch (error) {
    console.error("[DELIVERY-VERIFY-TOKEN] \u274C Token verification failed:", {
      jwtSecret: JWT_SECRET3.substring(0, 10) + "...",
...
function requireDeliveryAuth() {
  return (req, res, next) => {
    const authHeader = req.headers.authorization;
    console.log("[DELIVERY-AUTH] \u{1F510} Auth check:", {
      url: req.path,
    ...
    const token = authHeader.substring(7);
    console.log("[DELIVERY-AUTH] \u{1F50D} Token details:", {
      url: req.path,
      tokenLength: token.length,
...
var JWT_SECRET3, JWT_EXPIRES_IN3, REFRESH_TOKEN_EXPIRES_IN3;
var init_deliveryAuth = __esm({
  "server/deliveryAuth.ts"() {
    "use strict";
    JWT_SECRET3 = process.env.JWT_SECRET || "delivery-jwt-secret-change-in-production";
    JWT_EXPIRES_IN3 = "90d";
    REFRESH_TOKEN_EXPIRES_IN3 = "90d";
  }
});
...
__export(partnerAuth_exports, {
  ...
  hashPassword: () => hashPassword4,
  requirePartner: () => requirePartner,
  verifyPassword: () => verifyPassword4,
  verifyToken: () => verifyToken4
});
import jwt4 from "jsonwebtoken";
import bcrypt4 from "bcryptjs";
async function hashPassword4(password) {
  return bcrypt4.hash(password, 10);
}
async function verifyPassword4(password, hash) {
  return bcrypt4.compare(password, hash);
}
function generateAccessToken3(partner) {
  const payload = {
    partnerId: partner.id,
    chefId: partner.chefId,
    username: partner.username
  };
  return jwt4.sign(payload, JWT_SECRET4, { expiresIn: ACCESS_TOKEN_EXPIRY });
}
function generateRefreshToken4(partner) {
  const payload = {
    partnerId: partner.id,
    chefId: partner.chefId,
    username: partner.username
  };
  return jwt4.sign(payload, JWT_SECRET4, { expiresIn: REFRESH_TOKEN_EXPIRY });
}
function verifyToken4(token) {
  try {
    return jwt4.verify(token, JWT_SECRET4);
  } catch {
    return null;
  }
}
function requirePartner() {
  return (req, res, next) => {
    ...
    const token = authHeader.substring(7);
    const payload = verifyToken4(token);
    if (!payload) {
      res.status(401).json({ message: "Invalid or expired token" });
      return;
    }
...
var JWT_SECRET4, ACCESS_TOKEN_EXPIRY, REFRESH_TOKEN_EXPIRY;
...
async function broadcastPreparedOrderToAvailableDelivery(order) {
  ...
  console.log(`
\u{1F4E3} ========== BROADCAST TO AVAILABLE DELIVERY ==========`);
  ...
  console.log(`
\u{1F4CA} Total connected clients: ${clients.size}`);
  let deliveryClientCount = 0;
  const deliveryClientIds = [];
  clients.forEach((client, clientId) => {
    if (client.type === "delivery") {
      deliveryClientCount++;
...
\u2713 Connected delivery clients: ${deliveryClientCount}`);
  ...
  console.log(`
\u{1F50D} PHASE 1: Fetching delivery personnel details...`);
  ...
  if (connectedDeliveryIds.length > 0) {
    try {
      ...
      results.forEach((person, index2) => {
        const deliveryId = connectedDeliveryIds[index2];
        if (person) {
  ...
  console.log(`
\u{1F4E1} PHASE 2: Sending real-time notifications...`);
  for (const [deliveryPersonId, client] of Array.from(clients.entries())) {
    if (client.type === "delivery" && client.ws.readyState === WebSocket.OPEN) {
  ...
  console.log(`
\u{1F4BE} PHASE 3: Saving pending broadcasts for offline personnel...`);
  try {
    const activeDeliveryPersonnel = await storage2.getAvailableDeliveryPersonnel();
    let pendingSaved = 0;
...
async function sendEmail({ to, subject, html }) {
  ...
  console.log("\n" + "=".repeat(60));
  console.log("\u{1F4E7} [EMAIL] STEP 1\uFE0F\u20E3 - Starting email send");
  console.log("   To: " + to);
  console.log("   Subject: " + subject);
  console.log("   HTML Size: " + html.length + " chars");
  try {
    console.log("\u{1F4E7} [EMAIL] STEP 2\uFE0F\u20E3 - Calling Resend API...");
    ...
    console.log("\u{1F4E7} [EMAIL] STEP 3\uFE0F\u20E3 - Resend API Response:");
...
function createOrderConfirmationEmail(params) {
  ...
  return `
    ...
    <body>
      <div class="container">
        <!-- HEADER -->
        <div class="header">
          <h1 style="margin: 0;">\u{1F389} Welcome to RotiHai!</h1>
          <p style="margin: 5px 0;">Your Order is Confirmed</p>
        </div>

        <!-- ACCOUNT CREATION SECTION -->
        <div class="account-info">
          <h3 style="margin-top: 0;">\u{1F464} Your Account Created</h3>
          ...
          <p style="font-size: 12px; color: #666;">
            \u{1F4A1} Save this password. You'll need it to login and track your orders.
          </p>
        </div>

        <!-- ORDER CONFIRMATION SECTION -->
        <div class="section">
          <h3 style="margin-top: 0;">\u{1F4E6} Your Order Details</h3>
        ...
        <!-- DELIVERY DETAILS -->
        <div class="section">
          <h3 style="margin-top: 0;">\u{1F3E0} Delivery Address</h3>
          <p>${params.deliveryAddress}</p>
          ${params.estimatedDeliveryTime ? `<p><strong>\u23F1\uFE0F Estimated Delivery:</strong> ${params.estimatedDeliveryTime}</p>` : ""}
        </div>

        <!-- FOOTER -->
        <div style="text-align: center; padding: 20px; color: #666; font-size: 12px;">
          <p>Thank you for ordering with RotiHai! \u{1F64F}</p>
...
async function sendOrderConfirmationEmail(email, params) {
  return sendEmail({
    to: email,
    subject: `\u{1F389} Welcome & Order Confirmation - Order #${params.orderId}`,
...
\u{1F680} Please prepare and get ready for delivery!
...
function registerAdminRoutes(app2) {
  ...
  function getNextDeliveryDate2(fromDate, frequency, deliveryDays, maxDate) {
    const nextDate = new Date(fromDate);
    nextDate.setDate(nextDate.getDate() + 1);
    let attempts = 0;
    const maxAttempts = frequency === "monthly" ? 31 : 7;
    ...
    while (attempts < maxAttempts) {
      ...
      if (isDeliveryDay3(nextDate, frequency, deliveryDays)) {
        return nextDate;
      }
      nextDate.setDate(nextDate.getDate() + 1);
      attempts++;
  ...
  app2.post("/api/admin/auth/login", async (req, res) => {
    ...
    try {
      ...
      const tokenPayload = jwt5.verify(accessToken, process.env.JWT_SECRET || "admin-jwt-secret-change-in-production");
...
\u{1F4B3} ADMIN CONFIRMING PAYMENT FOR ORDER ${orderId}`);
      ...
      let refreshToken;
      if (!order.userId) {
        console.log(`\u{1F4DD} Admin payment confirmed for new user order ${orderId} - Creating user account`);
...
Please prepare this order.`;
    ...
    try {
      ...
      if (status === "delivered" && subscription.remainingDeliveries > 0) {
        ...
        if (plan) {
          ...
          const nextDelivery = getNextDeliveryDate2(subscription.nextDeliveryDate, plan.frequency, deliveryDays, maxDate);
  ...
  app2.get("/api/admin/reports/rotihai-earnings", requireAdmin(), async (req, res) => {
    try {
      const { from, to } = req.query;
  ...
  app2.get("/api/admin/reports/chef-payout", requireAdmin(), async (req, res) => {
    try {
      const { from, to, chefId } = req.query;
  ...
  app2.post("/api/admin/payouts/mark-paid", requireAdmin(), async (req, res) => {
    try {
  ...
  app2.post("/api/admin/payouts/mark-paid-bulk", requireAdmin(), async (req, res) => {
    try {
  ...
  app2.get("/api/admin/payouts/:orderId", requireAdmin(), async (req, res) => {
    try {
      const { orderId } = req.params;
  ...
  app2.get("/api/admin/coupon-stats", requireAdmin(), async (req, res) => {
    try {
  ...
  app2.get("/api/payment-settings", async (req, res) => {
    try {
  ...
  app2.post("/api/admin/referrals/expire", requireAdminOrManager(), async (req, res) => {
    try {
      console.log(`\u{1F550} [ADMIN] Manual trigger: Expiring old pending referrals`);
...
\u{1F4E2} STAGE 2: Order status changed to "prepared" - Checking delivery assignment...`);
  ...
  \u2705 ORDER ALREADY ASSIGNED`);
            console.log(`  Assigned to: ${updatedOrder.deliveryPersonName} (${updatedOrder.deliveryPersonPhone})`);
            console.log(`  Action: Sending update to ONLY this delivery person via broadcastOrderUpdate()`);
            console.log(`  Reason: They were assigned before "prepared" status, so only they need to know food is ready
`);
          } else {
            console.log(`
  \u{1F513} NO DELIVERY PERSON ASSIGNED YET`);
            console.log(`  Action: Broadcasting to ALL available delivery personnel`);
            console.log(`  Reason: Order is not claimed yet, so all available should be notified to compete for it
`);
            await broadcastPreparedOrderToAvailableDelivery(updatedOrder);
          }
          try {
...
function registerDeliveryRoutes(app2) {
  ...
  app2.post("/api/delivery/login", async (req, res) => {
    try {
      ...
      const isValid = await verifyPassword3(password, deliveryPerson.passwordHash);
      if (!isValid) {
        res.status(401).json({ message: "Invalid phone or password" });
        return;
      }
  ...
  app2.post("/api/delivery/auth/refresh", async (req, res) => {
    try {
      ...
      const payload = verifyToken3(refreshToken);
      if (!payload) {
        res.status(401).json({ message: "Invalid refresh token" });
        return;
      }
      ...
      const newRefreshToken = generateRefreshToken3(deliveryPerson);
      res.json({
...
async function registerRoutes(app2) {
  ...
  app2.use("/api/payments", gpay_verification_default);
  ...
  app2.post("/api/user/register", async (req, res) => {
    try {
      ...
      const passwordHash = await hashPassword2(result.data.password);
  ...
  app2.post("/api/user/login", async (req, res) => {
    try {
      ...
      const passwordMatch = await verifyPassword2(password, user.passwordHash);
      if (!passwordMatch) {
        res.status(401).json({ message: "Invalid phone number or password" });
        return;
      }
  ...
  app2.post("/api/subscriptions/:id/skip-delivery", requireUser(), async (req, res) => {
    try {
      ...
      if (!deliveryDate) return res.status(400).json({ message: "Delivery date required" });
      const subscription = await storage.getSubscription(subscriptionId);
  ...
  app2.post("/api/orders", async (req, res) => {
    try {
      ...
      if (referralCodeInput) {
        console.log(`\u{1F4F1} Received referral code in order request: ${referralCodeInput}`);
      }
...
\u{1F4B3} \u26A0\uFE0F [ATOMIC-PHASE2] Starting atomic payment confirmation + wallet deduction`);
        ...
        if (updatedOrder) {
          console.log(`
\u{1F4E3} PAYMENT CONFIRMED - BROADCASTING TO CHEF & ALL AVAILABLE DELIVERY PERSONNEL`);
          console.log(`
...
`);
      ...
      if (updatedOrder?.phone) {
        try {
          ...
          const pendingIds = pendingForPhone.filter((pc) => pc.status === "pending" && pc.id !== updatedOrder.id).map((pc) => pc.id);
...
Please accept and start preparation.`;
  ...
  app2.post("/api/orders/:id/cancel", async (req, res) => {
    try {
      ...
      console.log(`
\u{1F504} CANCEL REQUEST: Attempting to cancel order ${id}`);
      const order = await storage.getOrderById(id);
      if (!order) {
      ...
      console.log(`\u{1F4CB} Order found - Current Status: ${order.status}, Payment: ${order.paymentStatus}`);
  ...
  app2.post("/api/pending-checkouts", async (req, res) => {
    try {
  ...
  app2.get("/api/pending-checkouts/by-phone/:phone", async (req, res) => {
    try {
  ...
  app2.delete("/api/admin/pending-checkouts/:id", async (req, res) => {
    try {
      const { id } = req.params;
...
\u{1F5FA}\uFE0F [DEBUG] /api/chefs/by-location called:`);
      ...
      const nearbyChefs = chefsWithDistance.filter((chef) => maxDeliveryDistance(chef)).sort((a, b) => (a.distanceFromUser || 0) - (b.distanceFromUser || 0));
...
-RotiHai Admin System
  ...
  setInterval(async () => {
    ...
    if (hours === 2 && minutes === 0) {
      console.log(`\u{1F550} [SCHEDULER] Running daily referral expiration cleanup at ${now.toISOString()}`);
      try {
        const expiredCount = await storage.expireOldPendingReferrals();
...
app.use((req, res, next) => {
  if (enableVite2) {
    ...
    res.set({
      ...
      "Expires": "-1",
      "ETag": Date.now().toString()
      // Force revalidation every second
    });
...
(async () => {
  ...
  app.post("/api/partner/auth/login", async (req, res) => {
    try {
      ...
      const isValid = await verifyPassword4(password, partner.passwordHash);
      ...
      const accessToken = generateAccessToken3(partner);
      const refreshToken = generateRefreshToken4(partner);
  ...
  app.post("/api/partner/auth/refresh", async (req, res) => {
    try {
      ...
      const newAccessToken = generateAccessToken3(partner);
  ...
  app.put("/api/partner/change-password", requirePartner(), async (req, res) => {
    try {
      ...
      const isValid = await verifyPassword4(currentPassword, partner.passwordHash);
      if (!isValid) {
        res.status(401).json({ message: "Current password is incorrect" });
        return;
      }
